# Проверка performance budget

Пороговые значения взяты из `budgets.json`. Значения рассчитаны как медиана по Lighthouse CI JSON-отчетам.

| Маршрут | Показатель | Значение | Порог | Статус |
| --- | --- | ---: | ---: | --- |
| /dashboard | LCP | 372 мс | <= 2500 мс | выполнен |
| /dashboard | TBT | 272 мс | <= 200 мс | превышен |
| /dashboard | CLS | 0.000 | <= 0.1 | выполнен |
| /dashboard | Script transfer | 67 КБ | <= 300 КБ | выполнен |
| /dashboard | Image transfer | 0 КБ | <= 200 КБ | выполнен |
| /dashboard | Total transfer | 72 КБ | <= 700 КБ | выполнен |
| /dashboard | Total requests | 7 | <= 50 | выполнен |
| /details/line-a | LCP | 411 мс | <= 2500 мс | выполнен |
| /details/line-a | TBT | 296 мс | <= 200 мс | превышен |
| /details/line-a | CLS | 0.001 | <= 0.1 | выполнен |
| /details/line-a | Script transfer | 68 КБ | <= 300 КБ | выполнен |
| /details/line-a | Image transfer | 0 КБ | <= 200 КБ | выполнен |
| /details/line-a | Total transfer | 73 КБ | <= 700 КБ | выполнен |
| /details/line-a | Total requests | 8 | <= 50 | выполнен |
| /events | LCP | 370 мс | <= 2500 мс | выполнен |
| /events | TBT | 288 мс | <= 200 мс | превышен |
| /events | CLS | 0.037 | <= 0.1 | выполнен |
| /events | Script transfer | 66 КБ | <= 300 КБ | выполнен |
| /events | Image transfer | 0 КБ | <= 200 КБ | выполнен |
| /events | Total transfer | 71 КБ | <= 700 КБ | выполнен |
| /events | Total requests | 6 | <= 50 | выполнен |
| /reports | LCP | 411 мс | <= 2500 мс | выполнен |
| /reports | TBT | 305 мс | <= 200 мс | превышен |
| /reports | CLS | 0.000 | <= 0.1 | выполнен |
| /reports | Script transfer | 67 КБ | <= 300 КБ | выполнен |
| /reports | Image transfer | 0 КБ | <= 200 КБ | выполнен |
| /reports | Total transfer | 72 КБ | <= 700 КБ | выполнен |
| /reports | Total requests | 7 | <= 50 | выполнен |
| /settings | LCP | 363 мс | <= 2500 мс | выполнен |
| /settings | TBT | 0 мс | <= 200 мс | выполнен |
| /settings | CLS | 0.000 | <= 0.1 | выполнен |
| /settings | Script transfer | 63 КБ | <= 300 КБ | выполнен |
| /settings | Image transfer | 0 КБ | <= 200 КБ | выполнен |
| /settings | Total transfer | 67 КБ | <= 700 КБ | выполнен |
| /settings | Total requests | 5 | <= 50 | выполнен |
